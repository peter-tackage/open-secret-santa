...
public class DefaultNotifyExecutor implements NotifyExecutor {
    ...
    public Observable<NotifyStatusEvent> notifyDraw(final Group group, final long[] memberIds) {
        return Observable.create(new Observable.OnSubscribe<NotifyStatusEvent>() {
            @Override
            public void call(Subscriber<? super NotifyStatusEvent> subscriber) {
                ...
                for (long memberId : memberIds) {
                    ...
                    if (assignment == null) {
                        ...
                        subscriber.onError(new Exception("No Assignment for Member: " + member.getName()));
                        return;
                    }
                    ...
                    switch (member.getContactMethod()) {
                        case SMS:
                            ...
                            subscriber.onNext(new NotifyStatusEvent(assignment));
                        ...
                        case EMAIL:
                            ...
                            if (mAuth.getEmailAuth() == null) {
                                ...
                                mDb.update(assignment);
                                subscriber.onNext(new NotifyStatusEvent(assignment));
                                subscriber.onError(new Exception("Error - Not authorized to use your email account"));
                                return;
                            }
