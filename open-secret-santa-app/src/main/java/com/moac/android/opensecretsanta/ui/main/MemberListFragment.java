...
public class MemberListFragment extends ListFragment {
    ...
    private void bindViews(View view) {
        mAutoCompleteTextView = (AutoCompleteTextView) view
                .findViewById(R.id.autoCompleteTextView_addMember);
    }
    ...
    private void attemptDraw() {
        try {
            ...
            Toast.makeText(getActivity(), getString(R.string.draw_engine_init_error_msg),
    ...
    private void configureViews() {
        ...
        getListView().setMultiChoiceModeListener(new AbsListView.MultiChoiceModeListener() {
            ...
            public void onItemCheckedStateChanged(ActionMode mode, int position, long id,
                ...
                mode.setTitle(
                        String.format(getString(R.string.list_selection_count_title_unformatted),
                                      checkedItemCount));
                ...
                mode.getMenu().findItem(R.id.menu_item_notify_selection)
                    .setVisible(mMode == Mode.Notify && hasSendableItemChecked);
                mode.getMenu().findItem(R.id.menu_item_restrict)
                    .setVisible(mMode == Mode.Building && isSingleItemChecked);
                mode.getMenu().findItem(R.id.menu_item_reveal)
                    .setVisible(mMode == Mode.Notify && isSingleItemChecked);
            }
        });
    ...
    private void confirmDeleteMembers(final long[] memberIds) {
        String memberQuantity = getResources()
                .getQuantityString(R.plurals.memberQuantity, memberIds.length);
        new AlertDialog.Builder(getActivity())
                .setTitle(getString(R.string.delete_selection_dialog_title_unformatted,
                                    memberIds.length,
                                    memberQuantity))
                ...
                .setMessage(
                        getString(R.string.delete_members_confirm_msg_unformatted, memberQuantity))
                .create()
                .show();
    }
    ...
    private void openRenameGroupDialog() {
        ...
        okButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                GroupNameValidator validator = new GroupNameValidator(mDb, mGroup.getId(),
                                                                      newGroupName);
    ...
    private void setMenuItemsForMode(Mode mode) {
        ...
        if (mMenu != null) {
            ...
            mMenu.findItem(R.id.menu_item_notify_group)
                 .setVisible(mode == Mode.Notify && hasSendableItem(mAdapter));
    ...
    private void doNotify(SparseBooleanArray checkedMemberPositions) {
        if (checkedMemberPositions == null || checkedMemberPositions.size() == 0) {
            return;
        }
        ...
        return ob.subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread()).
                subscribe(new Observer<DrawResultEvent>() {
                    ...
                    public void onError(Throwable e) {
                        Log.i(TAG, "onError");
                        mDrawProgressDialog.dismiss();
                        Toast.makeText(getActivity(), getString(R.string.draw_failed_msg),
                                       Toast.LENGTH_LONG).show();
                    }
                    ...
                    public void onNext(DrawResultEvent args) {
                        Log.i(TAG, "onNext");
                        Toast.makeText(getActivity(), getString(R.string.draw_success_msg),
    ...
    private void showDrawProgressDialog() {
        ...
        mDrawProgressDialog = ProgressDialog
                .show(getActivity(), "", getString(R.string.draw_in_progress_msg), true);
    }
    ...
    private void doReveal(long memberId) {
        ...
        if (receiver.getContactId() != PersistableObject.UNSET_ID
            && receiver.getLookupKey() != null) {
            Uri lookupUri = ContactsContract.Contacts
                    .getLookupUri(receiver.getContactId(), receiver.getLookupKey());
            Uri contactUri = ContactsContract.Contacts
                    .lookupContact(getActivity().getContentResolver(), lookupUri);
            avatarUri = contactUri.toString();
        }
        // Create an instance of the dialog fragment and show it
        DialogFragment dialog = RevealDialogFragment
                .create(giver.getName(), receiver.getName(), avatarUri);
    ...
    private void addMember(Member member, Group group) {
        ...
        if (existing != null) {
            msg = String
                    .format(getString(R.string.duplicate_name_msg_unformatted), member.getName());
        } else {
            long id = mDb.create(member);
            if (id != PersistableObject.UNSET_ID) {
                msg = String
                        .format(getString(R.string.member_add_msg_unformatted), member.getName());
                invalidateAssignments(group);
                populateMemberList();
            } else {
                msg = String.format(getString(R.string.failed_add_member_msg_unformatted),
                                    member.getName());
            }
        }
        ...
        try {
            ...
            if (!classname.equals(defaultName)) {
                ...
                try {
                    ...
                    Log.e(TAG, "Unable to initialise default draw engine class: " + classname,
                          ideexp2);
                    throw ideexp2;
                }
            }
    ...
    private static boolean hasSendableItemChecked(ListView list) {
        ...
        for (int i = 0; i < checkedPositions.size(); i++) {
            ...
            if (isChecked && ((MemberRowDetails) (list.getAdapter().getItem(position))).getMember()
                                                                                       .getContactMethod()
                                                                                       .isSendable()) {
